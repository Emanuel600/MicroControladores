...
void caractere_LCD(char character)
{
    ...
    for(i = 0 ; i < 5 ; i++) {
        fb[indice_fb] = ASCII[character -
                        0x20][i];//0x20 is the ASCII character for Space (' '). The font table starts with this character
        indice_fb++;
    }
...
//--------------------------------------------------------------------------------------------------------------
void desenha_circulo(int32_t x0, int32_t y0,
        int32_t radius,	// valores int se fazem necess�rio devido as compara��es
        uint32_t prop) // 0 =  paga pixel, 1 = liga pixel
{
...
*		------------p2
*/
void desenha_retangulo(struct  pontos_t* p,
        uint32_t prop)	/*  p.x1=x1, p.y1=y1, p.x2=x2, p.y2=y2, passagem dos pontos por struct	*/
...
//----------------------------------------------------------------------------------------------
void escreve_Nr_Peq(uint32_t x, uint32_t y, int32_t valor,
        uint32_t quant2Print) // quant2Print = 0, imprime todos os digitos
{
...
//----------------------------------------------------------------------------------------------
void Apaga_Figura(struct  pontos_t* p, const struct figura_t* f)
{
    uint32_t x, y, larg, alt, i, j, ajx, ajy;
