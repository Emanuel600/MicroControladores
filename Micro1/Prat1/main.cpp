...
int main()
{
    ...
    DDRD                = 0xFF; // Using all pins on port D as output
    PORTD               = 0xFF; // Set all pins to HIGH
    BUTTON_PORT         = 0X00; // Setting all pins as input for simplicity
    SetBit(PCICR, 0);           // Set to detect interrupt on PCINT0-7
    SetBit(PCMSK0, 0);          // Sets interrupt mask to enable only PCINT0 pin

    while(1) {
        PORTD   = hex2SevenSegments(counter, DISPLAY_COMMON_ANODE, 0);

        counter = ((counter == 0xF) & direction)  ? 0   : counter + 1;
        counter = ((counter == 0x0) & !direction) ? 0xF : counter - 1;
